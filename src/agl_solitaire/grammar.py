...
class RegularGrammar(FormalGrammar):
    ...
    def __str__(self):
        """Display the internals of the grammar."""
...
class PatternGrammar(FormalGrammar):
    ...
    def __str__(self):
        """Display the internals of the grammar."""
        # convert frozensets to basic sets for more aesthetic presentation
        plain_set_patterns = map(lambda x: list(map(set, x)), self.patterns)
        def pattern_to_str(i, t):
            return 'T' + str(i) + ' = ' + str(t)
        return '\n'.join([pattern_to_str(i, t) for i, t in enumerate(plain_set_patterns)])
